  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <Card className="bg-gradient-card border-construction-steel/30">
        <CardHeader>
          <CardTitle className="text-white">Total Maintenance Cost: ${Number(totalCost).toLocaleString()}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {rows.map((r) => (
            <div key={r.id} className="p-4 bg-construction-dark rounded-lg">
              <div className="flex items-center justify-between">
                <p className="text-white font-medium">{r.equipment?.name ?? "Unknown"}</p>
              ...
              <p className="text-white mt-2">{r.description}</p>
            </div>
          ))}
