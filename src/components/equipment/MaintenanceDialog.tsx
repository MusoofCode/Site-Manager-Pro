...
export function MaintenanceDialog({ open, onOpenChange, onSuccess }: Props) {
  ...
  const equipmentValue = watch("equipment_id");
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const { error } = await supabase.from("maintenance_logs").insert([payload as any]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-construction-slate border-construction-steel/30 text-white">
        <DialogHeader>
          <DialogTitle className="text-2xl">New Maintenance Log</DialogTitle>
        </DialogHeader>

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <Label>Equipment</Label>
            <Select value={equipmentValue} onValueChange={(v) => setValue("equipment_id", v)}>
            ...
            {errors.equipment_id && <p className="text-red-400 text-sm mt-1">{errors.equipment_id.message}</p>}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label>Cost ($)</Label>
              <Input type="number" step="0.01" {...register("cost")} className="bg-construction-dark border-construction-steel text-white" />
            </div>
          </div>

          <div>
            <Label>Description</Label>
            <Textarea rows={3} {...register("description")} className="bg-construction-dark border-construction-steel text-white" />
